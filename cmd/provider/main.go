...
func main() {
	var (
		app              = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Exoscale").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-jet-exoscale"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-jet-exoscale",
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add Exoscale APIs to scheme")
	kingpin.FatalIfError(controller.Setup(mgr, log, rl, setup, ws, pconfig.GetProvider(tf.Provider()), 1), "Cannot setup Exoscale controllers")
